...
public class InventoryItem : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    void Awake()
    {
        ...
        // Try to find quantity text in children if not assigned
        if (quantityText == null)
        {
            quantityText = GetComponentInChildren<Text>();
        }
        
        // If still no text, create one
        if (quantityText == null)
        {
    ...
    public void InitialiseItem(ItemScript newItem, int amount = 1)
    {
        ...
        // Fix the RectTransform to make it visible
    ...
    {
        // Check for slot under pointer
        GameObject slot = GetSlotUnderPointer(eventData);
        if (slot != null)
        {
            // Check if slot already has an InventoryItem (not counting other children like text)
            ...
            {
                // Slot occupied, return to original parent
                ReturnToParent();
            }
        }
        else
        {
            // No slot, return to original parent
            ReturnToParent();
        }
    }
